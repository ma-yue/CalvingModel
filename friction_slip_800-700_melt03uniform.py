...
B0 = 15.77                # time-dependent viscosity constant, Pa*day^1/3
C0 = 2.0E5                # Pa*m^(-1/3)*day^1/3
...
for step in range(steps):
    ...
    (u,p) = split(w)
    (v,q) = split(y)
    u_k = interpolate(Constant((0.01,0.0)),vector)
    ux_k = interpolate(Constant(0.01),scalar)
    uz_k = interpolate(Constant(0.0),scalar)
    p_k = interpolate(Constant(1.0E6),scalar)

    # Normal vector
    n = FacetNormal(mesh)
    # Tangential vector (just for bed)
    #tn = as_vector([n[1],-n[0]])
    tn = interpolate(Constant((1.0,0.0)),vector)

    f = Constant((0, -rho_i*g))     # gravity
    h = CellSize(mesh)
    beta  = 0.2
    delta = beta*h*h
    a = (inner(nabla_grad(v), nu(u_k)*epsilon(u)) - div(v)*p +\
         1.0E10*q*div(u) + delta*inner(nabla_grad(q), nabla_grad(p)))*dx
    L = inner(v + delta*nabla_grad(q), f)*dx - inner(waterpressure, v + delta*nabla_grad(q))*ds(5) - C0*inner(u_k,tn)*inner(tn, v+delta*nabla_grad(q))*ds(1)
    ...
    max = 50              # max number of iterations allowed
...
#        print "count = %d, error = %g" % (count,eps)
    ...
    # Full stress
    sigma = project(nu(u)*epsilon(u)-p*Identity(tensor.cell().topological_dimension()),tensor)
