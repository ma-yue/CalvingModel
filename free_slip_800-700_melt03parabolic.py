...
meltrate = 0.3            # m/day avg, 0 at water line and bed, max halfway, parabolic
...
#            f.write("\n".join(" ".join(map(str, x)) for x in (b0_x,b0_z,b1_x,b1_z,s0_x,s0_z,s1_x,s1_z)))
    ...
    domain_vertices = []           # a list to store final boundary points, in order to
...
#    x[pointer.index()] -= 2*meltrate*dt  # melt away the grounding line first
            ...
            if f.exterior():
                for v in vertices(f):
                    ...
                    if z[k] > depth:
                        ...
                        pointer = v
                        x[k] -= (1-z[k]/depth)*(z[k]/depth)*6*meltrate*dt
                        next = 1
                        break
            if next or end:
                break
    ...
    for v in bdry_order:
        ...
        if z[k] < depth and x[k] > 0.5*grounding:
            d = math.hypot(x[k]-grounding,z[k])
            if d > 1E-3 and d < 0.9*gridsize:
                continue
